...
export async function sendBlogApprovedEmail(to, name, blog) {
  const t = getTransporter();
  const blogUrl = `${process.env.NEXT_PUBLIC_APP_URL || "https://tpicpc.com"}/blogs/${blog._id}`;
  return t.sendMail({
    from: `"TPI CPC" <${process.env.EMAIL_USER || "noreply@tpicpc.com"}>`,
    to,
    subject: `🎉 Your blog "${blog.title}" is now published on TPI CPC`,
    text: `Hi ${name || "there"},

Great news! Your blog post has been approved by an admin and is now live on TPI CPC.

Title: ${blog.title}
Read it here: ${blogUrl}

Share it with friends, classmates, and on social media — every view, like, and comment helps the community grow.

Thanks for contributing to TPI CPC!
— The TPI CPC Team`,
    html: `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Blog published</title></head>
<body style="margin:0;padding:0;background:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#111827;">
  <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#f3f4f6;padding:32px 16px;">
    <tr><td align="center">
      <table cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:560px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(15,23,42,0.06);">
        <tr><td style="background:linear-gradient(135deg,#10b981 0%,#6366f1 100%);padding:28px;text-align:center;color:#fff;">
          <div style="font-size:13px;letter-spacing:2px;opacity:0.85;text-transform:uppercase;font-weight:600;">TPI CPC · Blog approved</div>
          <div style="font-size:26px;font-weight:800;margin-top:8px;">Your blog is published! 🎉</div>
        </td></tr>
        <tr><td style="padding:28px;">
          <p style="margin:0 0 16px;font-size:15px;color:#374151;">Hi <strong>${name || "there"}</strong>,</p>
          <p style="margin:0 0 20px;font-size:15px;color:#4b5563;line-height:1.6;">
            Great news — your blog post has been approved by an admin and is now live on TPI CPC for the whole community to read.
          </p>
          ${blog.image ? `<img src="${blog.image}" alt="${blog.title}" style="display:block;width:100%;max-height:220px;object-fit:cover;border-radius:12px;margin-bottom:16px;" />` : ""}
          <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:12px;padding:18px 20px;margin-bottom:24px;">
            <div style="font-size:11px;letter-spacing:1px;text-transform:uppercase;color:#6366f1;font-weight:700;">Now live</div>
            <div style="font-size:18px;font-weight:700;color:#111827;margin:6px 0 0;line-height:1.3;">${blog.title}</div>
          </div>
          <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
            <tr><td align="center" style="padding-bottom:16px;">
              <a href="${blogUrl}" style="display:inline-block;background:linear-gradient(135deg,#6366f1 0%,#8b5cf6 100%);color:#fff;text-decoration:none;font-weight:600;padding:13px 28px;border-radius:10px;font-size:15px;">Read your blog →</a>
            </td></tr>
          </table>
          <p style="margin:0;font-size:13px;color:#6b7280;line-height:1.6;">
            Thanks for sharing your knowledge with the TPI CPC community. Keep writing!
          </p>
        </td></tr>
        <tr><td style="background:#f9fafb;padding:18px;text-align:center;font-size:12px;color:#9ca3af;border-top:1px solid #e5e7eb;">
          <strong style="color:#374151;">TPI CPC</strong> — Computer & Programming Club
...
export async function sendEventAnnouncementEmail(to, name, event) {
  const t = getTransporter();
  const eventUrl = `${process.env.NEXT_PUBLIC_APP_URL || "https://tpicpc.com"}/events`;
...
A new TPI CPC event has just been announced!
...
<body style="margin:0;padding:0;background:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#111827;">
  <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#f3f4f6;padding:32px 16px;">
    <tr><td align="center">
      <table cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:560px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(15,23,42,0.06);">
        <tr><td style="background:linear-gradient(135deg,#6366f1 0%,#ec4899 100%);padding:28px;text-align:center;color:#fff;">
          <div style="font-size:13px;letter-spacing:2px;opacity:0.85;text-transform:uppercase;font-weight:600;">TPI CPC · ${event.eventType || "Event"}</div>
          <div style="font-size:26px;font-weight:800;margin-top:8px;line-height:1.2;">${event.title}</div>
        </td></tr>
        ${event.eventImage ? `<tr><td><img src="${event.eventImage}" alt="${event.title}" style="display:block;width:100%;max-height:240px;object-fit:cover;" /></td></tr>` : ""}
        <tr><td style="padding:28px;">
          <p style="margin:0 0 16px;font-size:15px;color:#374151;">Hi <strong>${name || "there"}</strong>,</p>
          <p style="margin:0 0 20px;font-size:15px;color:#4b5563;line-height:1.6;">
            We're excited to announce a brand new TPI CPC event. Save the date below!
          </p>
          <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:12px;padding:14px 0;margin-bottom:20px;">
            <tr><td style="padding:6px 20px;font-size:14px;color:#374151;">📅 <strong>When:</strong> ${when}</td></tr>
            <tr><td style="padding:6px 20px;font-size:14px;color:#374151;">📍 <strong>Where:</strong> ${event.location || "TBA"}</td></tr>
            ${event.organizer ? `<tr><td style="padding:6px 20px;font-size:14px;color:#374151;">👤 <strong>Organizer:</strong> ${event.organizer}</td></tr>` : ""}
          </table>
          ${event.description ? `<p style="margin:0 0 24px;font-size:14px;color:#4b5563;line-height:1.6;white-space:pre-line;">${event.description}</p>` : ""}
          <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
            <tr><td align="center" style="padding-bottom:8px;">
              <a href="${eventUrl}" style="display:inline-block;background:linear-gradient(135deg,#6366f1 0%,#8b5cf6 100%);color:#fff;text-decoration:none;font-weight:600;padding:13px 28px;border-radius:10px;font-size:15px;">View event details →</a>
            </td></tr>
          </table>
        </td></tr>
        <tr><td style="background:#f9fafb;padding:18px;text-align:center;font-size:12px;color:#9ca3af;border-top:1px solid #e5e7eb;">
          <strong style="color:#374151;">TPI CPC</strong> — Computer & Programming Club<br/>
          <span style="color:#9ca3af;">You received this because you're a registered member.</span>
        </td></tr>
      </table>
    </td></tr>
  </table>
